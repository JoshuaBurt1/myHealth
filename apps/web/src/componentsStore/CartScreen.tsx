  ...
  const CartScreen: React.FC<CartScreenProps> = ({ isEmbedded = false, onClose }) => {
    ...
    return (
      /* 1. Added h-full and overflow-hidden to the parent to contain the scroll */
      <div className={`${isEmbedded ? 'pt-0 px-4 pb-4 h-full' : 'p-6 h-screen'} bg-transparent flex flex-col overflow-hidden`}>
        ...
        {/* 2. Restored flex-1 and added overflow-y-auto here */}
        ...
        {/* 3. Footer remains fixed at the bottom because of flex-col + flex-1 above */}
        {cartItems.length > 0 && (
