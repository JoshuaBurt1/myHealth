...
import { collection, query, orderBy, onSnapshot, addDoc, updateDoc, doc, getDoc, setDoc, deleteDoc, serverTimestamp, Timestamp } from 'firebase/firestore';
...
const ForumScreen: React.FC = () => {
  ...
  // Track Both Login States
  const [userData, setUserData] = useState<any>(null);
  const [lastLogin, setLastLogin] = useState<Timestamp | null>(null);
  const [previousLogin, setPreviousLogin] = useState<Timestamp | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      if (userDoc.exists()) {
        ...
        setPreviousLogin(data.previous_login || null);

        // UPDATE FOR NEXT TIME: 
        // Move current last_login to previous_login, and set new last_login to now.
        await updateDoc(userRef, {
          previous_login: data.last_login || serverTimestamp(),
  ...
  // --- NEW: Mark Post As Read ---
  // Pass this into the PostCard to trigger when the user opens the reply threads
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const commonData: any = {
        authorId: user.uid, authorName: realName, createdAt: serverTimestamp(), lastUpdated: serverTimestamp(), lastUpdatedBy: user.uid,
    ...
    <div className="flex flex-col lg:flex-row gap-8 p-4 bg-slate-50 min-h-screen pb-24 max-w-7xl mx-auto">
      ...
      <div className={`flex-1 max-w-2xl w-full mx-auto lg:mx-0 transition-all duration-300 ${
        ...
        <header className="mb-6 flex flex-row items-start justify-between gap-4">
          <div>
            <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Health Forum</h1>
        ...
        {/* SECTION TABS */}
        <div className="flex gap-1 sm:gap-2 mb-4 bg-white p-1 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <>
              {/* NOTE: Pass markPostAsRead to the PostCard so the badge clears when clicked */}
              {paginatedPosts.map((post) => {
                const lastReadPost = userData?.[`last_read_post_${post.id}`];
                const postTime = post.lastUpdated?.toDate?.() || new Date(0);
                
                const isUnread = postTime > (lastReadPost?.toDate?.() || lastLogin?.toDate?.() || new Date(0));

                return (
                  <PostCard 
                    key={post.id} 
                    post={post} 
                    isUnread={isUnread} // <--- This "uses" the logic
                    onMarkRead={() => markPostAsRead(post.id)}
                  />
                );
              })}
