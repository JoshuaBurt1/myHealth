...
export const PostCard: React.FC<PostCardProps> = ({ post, isUnread, onMarkRead, isAutoExpanded }) => {
  ...
  useEffect(() => {
    // 1. Safety check: Exit if not a help post or if the current user isn't the author
    if (!post.help || !post.helpEndDate) return;

    // 2. THE DATATYPE SOLUTION: Normalize Firestore Timestamp to JS Date
    ...
    const checkExpiry = () => {
      ...
      // 3. THE OVERFLOW FIX: Max 32-bit integer is ~24.8 days
      ...
      if (timeRemaining <= 0) {
        ...
        // Within safe timeout range: Set the timer
