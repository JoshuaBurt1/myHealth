...
export const PostCard: React.FC<PostCardProps> = ({ post, isUnread, onMarkRead, isAutoExpanded }) => {
  ...
  // If a post is updated again by someone else, clear the optimistic read
  useEffect(() => {
  ...
  useEffect(() => {
    // Exit if not a help post or if the current user isn't the author
    if (!post.help || !post.helpEndDate) return;

    // Normalize Firestore Timestamp to JS Date
    const expiryDate = post.helpEndDate instanceof Timestamp 
      ? post.helpEndDate.toDate() 
      : new Date((post.helpEndDate as any).seconds * 1000);
    ...
    const autoPurgePost = async () => {
      try {
        ...
        await batch.commit();
        console.log(`Successfully purged: ${post.id}`);
      } catch (err) {
    ...
    const checkExpiry = () => {
      ...
      // OVERFLOW FIX: Max 32-bit integer is ~24.8 days
    ...
    return checkExpiry();
  }, [post.id, post.helpEndDate, post.help, post.authorId, post.forumSection]);
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    if (window.confirm("Delete this post?")) {
      try {
        ...
        // find all nested replies at once
        const allRepliesQuery = query(
          collectionGroup(db, 'myHealth_replies'),
          where('rootPostId', '==', post.id)
        );
        ...
        // Delete the read tracking fields from the user document
        const userRef = doc(db, 'users', user.uid);
        batch.update(userRef, {
          [`last_read_post_${post.id}`]: deleteField()
        });

        // Delete the parent post document
        const postRef = doc(db, 'myHealth_posts', post.id);
        batch.delete(postRef);

        // Delete the news document if applicable
        ...
        if (repliesSnapshot.size > 498) {
          throw new Error("Too many replies to delete in one go.");
        }
  ...
  const toggleReplies = (e: React.MouseEvent) => {
    ...
    // if opening the replies section and it has an unread notification, mark it as read
    if (!isExpanded) {
      handleMarkAsReadOptimistically();
    }
