...
export const ExpandableMap = ({ userLocation, mapZoom, activeSection, filteredPosts, showExpandButton = true }: any) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [mapFilter, setMapFilter] = useState('none');

  // Filter pins locally based on the dropdown selection
  ...
  // Compute Map Grid Cells dynamically based on filtered map pins
  ...
  return (
    <>
      {/* SMALL MAP PREVIEW */}
      <div className="relative h-full bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden shadow-inner z-0 group">
        
        {/* Wrap the button in a conditional check */}
        {showExpandButton && (
          <button 
