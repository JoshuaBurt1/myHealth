...
export const ModalVitals: React.FC<ModalVitalsProps> = ({ 
  ...
  // Filter out standard vitals that are already tracked (either in DB or just added in this session)
  ...
  const updateNewEntryValue = (key: string, newValue: string) => {
    ...
    // Ensure we only process non-negative numbers
    ...
    try {
      ...
      // CRITICAL: Clear states BEFORE the alert to remove highlighted fields and prevent duplication
      ...
      alert(`Vitals saved successfully!${isEligibleForGems ? ' +10 gems earned!' : ''}`);
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {/* Bottom Section: The 4-Column Grid of Vitals */}
          <div>
            ...
            {(dynamicVitals.length === 0 && entries.length === 0) ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 sm:gap-4 gap-2">
                ...
                {/* 2. Brand New Vitals (Added in this session, pre-save) */}
                {entries
