...
// dropdown options for pain tracking
...
export const ModalVitals: React.FC<ModalVitalsProps> = ({
  ...
  const handleSaveVitals = async () => {
    // 1. Get all valid existing entries
    ...
    // 2. Get valid new entries, 
    const validNewEntries = entries.filter(e => {
      const isAlreadyTracked = validExistingEntries.some(exist => exist.key === e.key);
    ...
    try {
      ...
      // Process new entries and add them to the definitions registry
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[95vh]">        
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              {selectedCategory !== 'Custom' ? (
                ...
                <>
                  <input
                    type="text"
                    placeholder={
                      (trackedVitals.filter(v => v.isCustom).length + entries.filter(e => e.isCustom).length) >= 10
                      ? "Custom health metric limit (10) reached"
                      : "Vital Name (e.g. Sleep)"
                    }
                    disabled={(trackedVitals.filter(v => v.isCustom).length + entries.filter(e => e.isCustom).length) >= 10}
                    className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-slate-700 font-medium focus:outline-none focus:border-rose-500 disabled:bg-slate-50"
                    value={customName}
          ...
          {(() => {
            ...
            // 1. Create an O(1) index lookup map for the currently selected category tab
            ...
            // 2. Filter & sort existing vitals for the selected category
            const existingInCat = trackedVitals
              .filter(vital => {
                if (category === 'Custom') return vital.isCustom;
                return currentMap && Object.values(currentMap).includes(vital.key);
              })
              .sort((a, b) => getPos(a.key) - getPos(b.key));

            // 3. Filter & sort new session entries for the selected category
